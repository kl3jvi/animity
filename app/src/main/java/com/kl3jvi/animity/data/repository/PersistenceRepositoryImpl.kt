...
class PersistenceRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun getEpisodeContent(episodeUrl: String): Flow<EpisodeEntity> = episodeDao.getEpisodeContent(episodeUrl)

        override suspend fun isEpisodeOnDatabase(episodeUrl: String): Boolean = episodeDao.isEpisodeOnDatabase(episodeUrl)

        override suspend fun insertLocalEpisode(localEpisode: LocalEpisode) = localEpisodeDao.insert(localEpisode)

        override suspend fun incrementDownloadedEpisodesCount(animeId: Int) = localAnimeDao.incrementDownloadedEpisodesCount(animeId)

        override suspend fun insertLocalAnime(localAnime: LocalAnime) = localAnimeDao.insert(localAnime)

        override suspend fun getLocalAnimeById(aniListId: Int): LocalAnime? = localAnimeDao.getAnimeById(aniListId)
